...
struct TransientError : std::runtime_error { using std::runtime_error::runtime_error; };
...
TEST(RetryPolicyTest, RetryOnMatchingExceptionRetries) {
    ...
    auto result = with_retry(cfg, [&] {
        if (++calls < 3) throw TransientError{"transient"};
        return calls;
    });
    EXPECT_EQ(result, 3);
}
...
TEST(RetryPolicyTest, RetryOnNonMatchingExceptionPropagatesImmediately) {
    ...
    EXPECT_THROW(
        with_retry(cfg, [&] { ++calls; throw FatalError{"fatal"}; return 0; }),
        FatalError);
...
TEST(RetryPolicyTest, FullJitterStaysWithinBounds) {
    ...
    EXPECT_THROW(
        with_retry(cfg, [] { throw std::runtime_error{"x"}; return 0; }),
        RetryExhausted);
...
TEST(RetryPolicyTest, DeadlineNeverDoesNotAffectRetry) {
    ...
    auto r = with_retry(cfg, [&] { ++calls; return 42; });
    EXPECT_EQ(r, 42);
    EXPECT_EQ(calls, 1);
}
