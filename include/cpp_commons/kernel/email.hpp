...
public:
    [[nodiscard]] static Result<Email, errors::ValidationError> parse(std::string_view input) {
        if (input.empty())
            return Result<Email, errors::ValidationError>::err(errors::ValidationError{"Email must not be empty"});

        auto at = input.find('@');
        if (at == std::string_view::npos || at == 0 || at == input.size() - 1)
            return Result<Email, errors::ValidationError>::err(errors::ValidationError{"Email must contain '@' with local and domain parts"});

        auto domain = input.substr(at + 1);
        if (domain.find('.') == std::string_view::npos)
            return Result<Email, errors::ValidationError>::err(errors::ValidationError{"Email domain must contain '.'"});

        // Reject obvious invalid chars (simplified RFC 5322)
        for (char c : input) {
            if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
                return Result<Email, errors::ValidationError>::err(errors::ValidationError{"Email must not contain whitespace"});
        }
