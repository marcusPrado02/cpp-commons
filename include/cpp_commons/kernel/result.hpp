...
public:
    ...
    template <typename F>
    auto and_then(F&& fn) & {
        return inner_.and_then(std::forward<F>(fn));
    }
    template <typename F>
    ...
    template <typename F>
    auto or_else(F&& fn) & {
        return inner_.or_else(std::forward<F>(fn));
    }
    template <typename F>
    auto map(F&& fn) & {
        return inner_.map(std::forward<F>(fn));
    }
    template <typename F>
    auto map(F&& fn) const& {
        return inner_.map(std::forward<F>(fn));
    ...
    auto map_err(F&& fn) & {
        ...
        if (is_err())
            return Result<T, E2>::err(std::forward<F>(fn)(inner_.error()));
...
Result<T, E> flatten(Result<Result<T, E>, E> r) {
    if (r.is_err())
        return Result<T, E>::err(std::move(r).error());
