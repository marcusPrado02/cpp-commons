...
struct Argon2Error : std::runtime_error {
    explicit Argon2Error(int code) : std::runtime_error{argon2_error_message(code)} {}
};
...
public:
    ...
    explicit Argon2Hasher(uint32_t t_cost = kTimeCost, uint32_t m_cost = kMemoryCost,
                          uint32_t threads = kParallelism, uint32_t hash_len = kHashLen)
        : t_cost_{t_cost}, m_cost_{m_cost}, threads_{threads}, hash_len_{hash_len} {}
    ...
    [[nodiscard]] std::string hash(const std::string& password) const {
        ...
        std::size_t enc_len =
            argon2_encodedlen(t_cost_, m_cost_, threads_, kSaltLen, hash_len_, Argon2_id);
        std::string encoded(enc_len, '\0');
        int rc = argon2id_hash_encoded(t_cost_, m_cost_, threads_, password.data(), password.size(),
                                       salt.data(), kSaltLen, hash_len_, encoded.data(), enc_len);
        if (rc != ARGON2_OK)
    ...
    [[nodiscard]] bool verify(const std::string& encoded, const std::string& password) const {
