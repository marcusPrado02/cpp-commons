...
public:
    ...
    InstrumentedUseCase(UseCase<In, Out, Err>& inner, Metrics& metrics, std::string name)
        : inner_{inner},
          metrics_{metrics},
          calls_{name + ".calls"},
          errors_{name + ".errors"},
...
template <typename In, typename Out, typename Err, kernel::MetricsPort Metrics>
InstrumentedUseCase(UseCase<In, Out, Err>&, Metrics&,
                    std::string) -> InstrumentedUseCase<In, Out, Err, Metrics>;
