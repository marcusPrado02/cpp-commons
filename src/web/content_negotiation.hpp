...
[[nodiscard]] inline std::string negotiate_content_type(
    ...
    const std::vector<std::string>& supported)
{
    ...
    struct AcceptEntry { std::string type; double q; };
    std::vector<AcceptEntry> entries;

    std::size_t pos = 0;
    while (pos < accept_header.size()) {
        auto comma = accept_header.find(',', pos);
        auto token = accept_header.substr(pos, comma == std::string_view::npos
        ...
        if (semi != std::string_view::npos) {
            ...
            while (!param.empty() && param.front() == ' ') param.remove_prefix(1);
            if (param.substr(0, 2) == "q=") {
                try { q = std::stod(std::string{param.substr(2)}); } catch (...) {}
            }
...
inline Middleware content_negotiation_middleware(
    std::vector<std::string> supported = {"application/json"})
{
    return [sup = std::move(supported)](const HttpRequest& req, const Handler& next) -> HttpResponse {
        auto accept = req.header("Accept").value_or("*/*");
        auto content_type = negotiate_content_type(accept, sup);

        if (content_type.empty()) {
            return HttpResponse{406, {{"Content-Type", "text/plain"}},
