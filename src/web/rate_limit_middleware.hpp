...
inline Middleware rate_limit_middleware(double max_tokens, double refill_rate,
                                        std::string retry_after_seconds = "1") {
    auto limiter = std::make_shared<resilience::RateLimiter>(max_tokens, refill_rate);

    return [limiter, retry = std::move(retry_after_seconds)](const HttpRequest& req,
                                                             const Handler& next) -> HttpResponse {
        if (!limiter->try_acquire()) {
            return HttpResponse{
                429,
                {{"Retry-After", retry}, {"Content-Type", "application/problem+json"}},
