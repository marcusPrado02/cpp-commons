...
inline Handler health_live_handler() {
    return [](const HttpRequest&) -> HttpResponse {
        return HttpResponse{200, {{"Content-Type", "application/json"}}, R"({"status":"up"})"};
    };
}

inline Handler health_ready_handler(std::shared_ptr<observability::HealthRegistry> registry) {
    return [reg = std::move(registry)](const HttpRequest&) -> HttpResponse {
        ...
        for (const auto& c : report.checks) {
            ...
            switch (c.status) {
                case observability::HealthStatus::Up:
                    checks_json += "up";
                    break;
                case observability::HealthStatus::Degraded:
                    checks_json += "degraded";
                    break;
                case observability::HealthStatus::Down:
                    checks_json += "down";
                    break;
            }
